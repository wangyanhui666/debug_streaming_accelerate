...
def main(args):
    ...
    for epoch in range(first_epoch, args.num_epochs):
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            noise1 = torch.randn(clean_images.shape, dtype=weight_dtype, device=clean_images.device)
            noise2 = torch.randn(clean_images.shape, dtype=weight_dtype, device=clean_images.device)
            bsz = clean_images.shape[0]
            # Sample a random timestep for each image
            timesteps1 = torch.randint(
                0, noise_scheduler1.config.num_train_timesteps, (bsz,), device=clean_images.device
            ).long()
            timesteps2 = torch.randint(
                0, noise_scheduler2.config.num_train_timesteps, (bsz,), device=clean_images.device
            ).long()


            # Add noise to the clean images according to the noise magnitude at each timestep
            # (this is the forward diffusion process)
            noisy_images1 = noise_scheduler1.add_noise(clean_images, noise1, timesteps1)
            noisy_images2 = noise_scheduler2.add_noise(clean_images, noise2, timesteps2)

            with accelerator.accumulate(model1):
                # Predict the noise residual
                model1_output = model1(noisy_images1, timesteps1, class_labels, return_dict=False)[0]
                model2_input = torch.cat([noisy_images2,model1_output],dim=1)
                model2_output = model2(model2_input, timesteps2, class_labels, return_dict=False)[0]
                if args.prediction_type == "epsilon":
                    loss = F.mse_loss(model2_output.float(), noise2.float())  # this could have different weights!
                elif args.prediction_type == "sample":
                    alpha_t = _extract_into_tensor(
                        noise_scheduler2.alphas_cumprod, timesteps2, (clean_images.shape[0], 1, 1, 1)
                    )
                    snr_weights = alpha_t / (1 - alpha_t)
                    # use SNR weighting from distillation paper
                    loss = snr_weights * F.mse_loss(model2_output.float(), clean_images.float(), reduction="none")
                    loss = loss.mean()
                else:
