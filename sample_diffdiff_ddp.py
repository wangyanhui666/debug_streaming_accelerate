...
def main(args):
    ...
    if args.use_ema:
        model1=DiTTransformer2DModel.from_pretrained(args.ckpt, subfolder="transformer1_ema")
        model2=DiTTransformer2DModel.from_pretrained(args.ckpt, subfolder="transformer2_ema")
    else:
        model1=DiTTransformer2DModel.from_pretrained(args.ckpt, subfolder="transformer1")
        model2=DiTTransformer2DModel.from_pretrained(args.ckpt, subfolder="transformer2")
    model1.to(device)
    model2.to(device)
    model1.eval()  # important!
    model2.eval()  # important!
    vae = AutoencoderKL.from_pretrained(args.vae_path).to(device)
    noise_scheduler1 = DDPMScheduler(clip_sample=False)
    noise_scheduler2 = DDPMScheduler(clip_sample=False)
    if args.dtype == "fp32":
        dtype = torch.float32
    ...
    pipeline = DiffDiffPipeline(model1,model2, vae, noise_scheduler1,noise_scheduler2).to(dtype=dtype)
    ...
    inference_folder_name = f'inference_{checkpoint_name}-ema-{args.use_ema}-image_size-{args.image_size}-cfg-{args.cfg_scale}-step1-{args.num_sampling_steps1}-step2-{args.num_sampling_steps2}-dtype-{args.dtype}-seed-{args.global_seed}'
    ...
    for _ in pbar:
        ...
        images = pipeline(
            ...
            guidance_scale1=1.0,
            guidance_scale2=1.0,
            num_inference_steps1=args.num_sampling_steps1,
            num_inference_steps2=args.num_sampling_steps2,
...
if __name__ == "__main__":
    ...
    parser.add_argument("--num-sampling-steps1", type=int, default=2)
    parser.add_argument("--num-sampling-steps2", type=int, default=50)
