...
from diffusion_diffusers.model import DiT_XL_2, DiT_S_2
...
def evaluate(args, epoch, pipeline):
    ...
    image_grid_1.save(f"{test_dir}/{epoch:04d}_{8}_{50}.png")
    ...
    test_dir = os.path.join(args.output_dir, "samples")
    os.makedirs(test_dir, exist_ok=True)
    image_grid_2.save(f"{test_dir}/{epoch:04d}_{1}_{50}.png")

    images = pipeline(
        class_labels=class_labels,
        guidance_scale1=1.0,
        guidance_scale2=1.0,
        num_inference_steps1=2,
        num_inference_steps2=50,
        generator=torch.Generator(device='cpu').manual_seed(args.seed), # Use a separate torch generator to avoid rewinding the random state of the main training loop
    ).images
    # Make a grid out of the images
    ...
    test_dir = os.path.join(args.output_dir, "samples")
    os.makedirs(test_dir, exist_ok=True)
    image_grid_3.save(f"{test_dir}/{epoch:04d}_{2}_{50}.png")
    del pipeline
...
def parse_args():
    ...
    parser.add_argument("--ddpm_num_steps1", type=int, default=1000)
    parser.add_argument("--ddpm_num_steps2", type=int, default=1000)
...
def main(args):
    ...
    if version.parse(accelerate.__version__) >= version.parse("0.16.0"):
        ...
        def save_model_hook(models, weights, output_dir):
            if accelerator.is_main_process:
                if args.use_ema:
                    ema_model1.save_pretrained(os.path.join(output_dir, "transformer1_ema"))
                    ema_model2.save_pretrained(os.path.join(output_dir, "transformer2_ema"))

                for i, model in enumerate(models):
                    model.save_pretrained(os.path.join(output_dir, f"transformer{i+1}"))
        ...
        def load_model_hook(models, input_dir):
            if args.use_ema:
                load_model1 = EMAModel.from_pretrained(os.path.join(input_dir, "transformer1_ema"), model_cls=DiTTransformer2DModel)
                load_model2 = EMAModel.from_pretrained(os.path.join(input_dir, "transformer2_ema"), model_cls=DiTTransformer2DModel)
                ema_model1.load_state_dict(load_model1.state_dict())
                ema_model2.load_state_dict(load_model2.state_dict())
                ema_model1.to(accelerator.device)
    ...
    if args.model_config_name_or_path is None:
        model1 = DiT_XL_2(sample_size=args.resolution,in_channels=4,out_channels=4)
        model2 = DiT_S_2(sample_size=args.resolution,in_channels=8,out_channels=4)
    else:
    ...
    if args.enable_xformers_memory_efficient_attention:
        if is_xformers_available():
            ...
            model2.enable_xformers_memory_efficient_attention()
        else:
            raise ValueError("xformers is not available. Make sure it is installed correctly")

    if args.gradient_checkpointing:
        model1.enable_gradient_checkpointing()
    ...
    model1, model2, optimizer, lr_scheduler = accelerator.prepare(
        model1, model2, optimizer, lr_scheduler
    )
    ...
    for epoch in range(first_epoch, args.num_epochs):
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            noise1 = torch.randn(clean_latents.shape, dtype=weight_dtype, device=device)
            noise2 = torch.randn(clean_latents.shape, dtype=weight_dtype, device=device)
            bsz = clean_latents.shape[0]
            # Sample a random timestep for each image
            timesteps1 = torch.randint(
                0, noise_scheduler1.config.num_train_timesteps, (bsz,), device=device
            ).long()
            timesteps2 = torch.randint(
                0, noise_scheduler2.config.num_train_timesteps, (bsz,), device=device
            ).long()


            # Add noise to the clean images according to the noise magnitude at each timestep
            # (this is the forward diffusion process)
            noisy_images1 = noise_scheduler1.add_noise(clean_latents, noise1, timesteps1).to(dtype=weight_dtype)
            noisy_images2 = noise_scheduler2.add_noise(clean_latents, noise2, timesteps2).to(dtype=weight_dtype)

            with accelerator.accumulate(model1):
                # Predict the noise residual
                model1_output = model1(noisy_images1, timesteps1, labels, return_dict=False)[0]
                model2_input = torch.cat([noisy_images2,model1_output],dim=1)
                model2_output = model2(model2_input, timesteps2, labels, return_dict=False)[0]
                if args.prediction_type == "epsilon":
                    loss = F.mse_loss(model2_output.float(), noise2.float())  # this could have different weights!
                elif args.prediction_type == "sample":
                    alpha_t = _extract_into_tensor(
                        noise_scheduler2.alphas_cumprod, timesteps2, (clean_latents.shape[0], 1, 1, 1)
                    )
                    snr_weights = alpha_t / (1 - alpha_t)
                    # use SNR weighting from distillation paper
                    loss = snr_weights * F.mse_loss(model2_output.float(), clean_latents.float(), reduction="none")
                    loss = loss.mean()
                else:
            ...
            if accelerator.sync_gradients:
                if args.use_ema:
                    ema_model1.step(model1.parameters())
                    ema_model2.step(model2.parameters())
                progress_bar.update(1)
                global_step += 1
            ...
            if args.use_ema:
                logs["ema_decay1"] = ema_model1.cur_decay_value
                logs["ema_decay2"] = ema_model2.cur_decay_value
        ...
        if accelerator.is_main_process:
            if epoch % args.save_images_epochs == 0 or epoch == args.num_epochs - 1:
                ...
                if args.use_ema:
                    ...
                    ema_model1.copy_to(transformer1.parameters())
                    ema_model2.copy_to(transformer2.parameters())

                pipeline = DiffDiffPipeline(transformer1=transformer1, transformer2=transformer2, vae=vae, scheduler1=noise_scheduler1,scheduler2=noise_scheduler2)
                ...
                if args.use_ema:
                    ema_model1.restore(transformer1.parameters())
                    ema_model2.restore(transformer2.parameters())
